...
const Countdown = ({ date }) => {
  ...
  const [days, setDays] = useState(
    Math.floor(timeUntilWedding / (1000 * 60 * 60 * 24))
  );
  const [hours, setHours] = useState(
    Math.floor((timeUntilWedding % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))
  );
  const [minutes, setMinutes] = useState(
    Math.floor((timeUntilWedding % (1000 * 60 * 60)) / (1000 * 60))
  );
  const [seconds, setSeconds] = useState(
    Math.floor((timeUntilWedding % (1000 * 60)) / 1000)
  );
  ...
  return (
    <div id="Countdown">
      ...
      {areMarried && (
        <h1>{"Happily Married! Thanks for sharing in our special day"}</h1>
      )}
    </div>
  );
};
